...
fn empty_query() {
    ...
    /*
    world
        .insert(shared, vec![(Pos(1., 2., 3.))])
        .into_iter()
        .for_each(|e| entities.push(*e));

    world
        .insert(shared, vec![(Rot(0.4, 0.5, 0.6))])
        .into_iter()
        .for_each(|e| entities.push(*e));
    */
    world
        .insert(shared, vec![(Pos(4., 5., 6.), Rot(0.4, 0.5, 0.6))])
        .into_iter()
        .for_each(|e| entities.push(*e));

    let mut query = <()>::query().filter(component::<Pos>() | component::<Rot>());
    let mut count = 0;
    query.iter_entities(&world).for_each(|(e, _)| {
